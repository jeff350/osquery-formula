...
We will see a quick setup to configure the ``osquery`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``osquery`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``osquery`` formulas under ``/srv/salt/osquery/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/osquery/parameters/dns:domain/
    ln -s dns:domain /srv/salt/osquery/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/osquery/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/osquery/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``osquery`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``osquery`` formulas under ``/srv/salt/osquery/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/osquery/parameters/roles
...
We create a configuration for the role ``osquery/server`` in ``/srv/salt/osquery/parameters/roles/osquery/server.yaml``:
...
We create another configuration for the role ``osquery/client`` in ``/srv/salt/osquery/parameters/roles/osquery/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles osquery/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles osquery/client
...
.. code-block:: console
    ...
    mv /srv/salt/osquery/parameters/dns:domain/  '/srv/salt/osquery/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-osquery-as-json:
      test.nop:
        - name: {{ osquery | json }}
...
Here is an example based on `template-formula/osquery/config/file.sls`_:
...
.. code-block:: sls
    ...
    osquery-config-file-file-managed:
      file.managed:
        - name: {{ osquery.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='osquery-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ osquery.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            osquery: {{ osquery | json }}

This ``sls`` file expose a ``osquery`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ osquery['winner'] }}
...
.. _template-formula/osquery/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/osquery/config/file.sls
